...
class MICCANlong(nn.Module):
    def __init__(self, in_channel, out_channel, n_layer):
        ...
        self.fc1 = nn.Linear(1 * 512 * 512, 120)
    ...
    def forward(self, x_under, xk_under, mask, rec=True):
        ...
        if rec:
            for i in range(self.nlayer):
                ...
                x_rec_dc = self.dc(mask, x_res, xk_under)
        ...
        # 分类前向传播
            ...
            x_rec_dc = self.conv1(x_rec_dc)
            x_rec_dc = x_rec_dc.view(-1, 1 * 512 * 512)
            x_rec_dc = F.relu(self.fc1(x_rec_dc))  # output(120)
            x_rec_dc = F.relu(self.fc2(x_rec_dc))  # output(84)
            x_rec_dc = self.fc3(x_rec_dc)
            return x_rec_dc
...
class inconv(nn.Module):
    def __init__(self, in_ch, out_ch, padding):
        ...
        self.conv_3 = nn.Conv2d(2, 64, 3, padding=padding)
...
class UNetCSE(nn.Module):
    def __init__(self, n_channels, n_classes):
        ...
        for param in self.trans1.parameters():
            param.requires_grad = False
        for param in self.trans2.parameters():
            param.requires_grad = False
        for param in self.trans3.parameters():
            param.requires_grad = False
        for param in self.trans4.parameters():
            param.requires_grad = False
...
class _DAHead(nn.Module):
    def __init__(self, in_channels, nclass, aux=True, norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        ...
        self.conv1 = nn.Conv2d(128, nclass, 1)
